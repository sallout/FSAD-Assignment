...
// CORS configuration - Allow both localhost and ngrok
app.use(cors({
  origin: [
    "http://localhost:3000", // Local development
    "https://flo-sanidinic-interpolatively.ngrok-free.dev" // Ngrok frontend
  ],
  credentials: true,
}));
...
// Admin: Get all lent/borrowed items with student info
app.get("/admin/lent-items", authenticateToken, authorizeRoles("admin"), async (req, res) => {
  try {
    console.log("Admin lent-items endpoint called by user:", req.user);
    const result = await pool.query(`
      SELECT 
        br.id AS request_id,
      ...
      ORDER BY br.requested_at DESC
    `);
    console.log("Lent items query result:", result.rows.length, "items found");
...
// Admin: Get ALL borrow requests for debugging
app.get("/admin/all-requests", authenticateToken, authorizeRoles("admin"), async (req, res) => {
  try {
    console.log("Debug: Fetching ALL borrow requests");
    const result = await pool.query(`
      SELECT 
        br.id AS request_id,
        u.name AS student_name,
      ...
      ORDER BY br.requested_at DESC
    `);
    console.log("Debug: All requests found:", result.rows.length);
    res.status(200).json(result.rows);
  } catch (error) {
...
// Get all borrowed equipment with student info (public endpoint)
